...
import uuid
from constants import intents, month_mapping, day_mapping, TEMP, days_arr, PENDING_CALLBACK, FAILED, DURATION_CONFLICT
import re
...
from ..services.app.models.forward_message import ForwardDetails
from ..services.app.models.message import Message
from ..services.app.models.user import User
from ..services.app.models.exceptions import DatesMismatchError, ReplyError


from constants import months_regex, days_regex, start_date_pattern, start_date_pattern_2, end_date_pattern, end_date_pattern_2, ddmm_start_date_pattern, ddmm_end_date_pattern, final_duration_extraction, start_day_pattern, end_day_pattern
...
class McDetails(Message):
    ...
    def __init__(self, sid, name, body, intent=intents['TAKE_MC']):
        super().__init__(sid, name, body, intent)
    ...
    def generate_base(self):
        ...
        if duration_c:
            ...
            # if there are specified dates and duration is wrong
                ...
                raise ReplyError(os.environ.get('DURATION_MISMATCH_SID'), self, content_variables, status=DURATION_CONFLICT) #TODO fix this!
    ...
    def set_start_end_date(self):
        ...
        if len(start_dates) > 1:
            content_variables = {
                '1': "start",
                '2': ', '.join(str(date) for date in start_dates)
            }
            raise ReplyError(os.environ.get('CONFLICTING_DATES_SID'), self, content_variables, status=DURATION_CONFLICT)
        if len(end_dates) > 1:
            content_variables = {
                '1': "end",
                '2': ', '.join(str(date) for date in end_dates)
            }
            raise ReplyError(os.environ.get('CONFLICTING_DATES_SID'), self, content_variables, status=DURATION_CONFLICT)
    ...
    def named_month_extraction(self):
        ...
        user_str = re.sub(months_regex, self.replace_with_full_month, self.body, flags=re.IGNORECASE)
    ...
    def named_day_extraction(self):
        ...
        user_str = re.sub(days_regex, self.replace_with_full_day, self.body, flags=re.IGNORECASE)
    ...
    #     '''This function gets a mc_details object and returns a confirmation message to the person taking the MC'''
