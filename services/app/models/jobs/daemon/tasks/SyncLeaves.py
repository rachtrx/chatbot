...
from models.jobs.base.constants import Status, OutgoingMessageData, MessageType
...
class SyncLeaves(TaskDaemon):
    ...
    def execute(self):
        err_msg = ''
        conn_err_months_del = set()
        conn_err_months_update = set() 

        self.session = Session()
        self.missing_job_user_ids = {}
        self.records = {
        ...
        for mm, yy in combined_mmyy_set: # contains the mm, yy that are >= ysterday
            ...
            affected_records = self.filter_affected_records(cur_az_df, cur_db_df)
            if not affected_records:
                continue

            dates_to_del, dates_to_update = affected_records
            ...
            if not dates_to_del.empty:
                ...
                try:
                    ...
                    self.handle_dates_del(dates_to_del, Status.COMPLETED)
                except AzureSyncError as e:
                    conn_err_months_del.add(f"{mm}/{yy}")
                    self.handle_dates_del(dates_to_del, Status.FAILED)
                    self.logger.error(e)
            ...
            if not dates_to_update.empty:
                ...
                try:
                    ...
                    self.handle_dates_update(dates_to_update, Status.COMPLETED)
                except AzureSyncError as e:
                    conn_err_months_update.add(f"{mm}/{yy}")
                    self.logger.error(e)
                    self.handle_dates_update(dates_to_update, Status.FAILED)

        self.send_notify_updates()

        if len(conn_err_months_del) > 0 or len(conn_err_months_update) > 0:
            err_msg += DaemonMessage.AZURE_CONN_FAILED
            if len(conn_err_months_del) > 0:
                err_msg += " Records in " + join_with_commas_and(list(conn_err_months_del)) + "failed to delete. "
    ...
    def filter_affected_records(self, cur_az_df, cur_db_df):
        ...
        return [dates_to_del, dates_to_update]
        
    def handle_dates_update(self, dates, status):
        add_grouped = dates.groupby(['_merge', 'user_id'], observed=True)
        for (_merge, user_id), group in add_grouped:
            ...
            records = self.session.query(
                ...
                LeaveRecord.sync_status != status # TODO should users be informed twice?
            ).all()

            update_data = [{'id': record.id, 'sync_status': status} for record in records]

            self.session.bulk_update_mappings(LeaveRecord, update_data)

            for record in records:
                self.update_records(Update.ADD, status, user_id, record.date)
        
    def handle_dates_del(self, dates_to_del, status):
        ...
        for (_merge, user_id), group in del_grouped:
            ...
            if _merge == "az_only": # blank row / no match with db. if no match and name: send a spearate message
                if user_id and not pd.isna(user_id) and len(del_dates) > 0:
                    if user_id not in self.missing_job_user_ids:
                        self.missing_job_user_ids[user_id] = []
                    self.missing_job_user_ids[user_id].extend(del_dates)
                continue
            ...
            for record in records:
                self.update_records(Update.DEL, status, user_id, record.date)
    
    def send_notify_updates(self):
